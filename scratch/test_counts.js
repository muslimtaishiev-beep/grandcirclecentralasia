...
const testsData: Record<number, TestData> = {
  "7": {
    grade: 7,
    russian: [
{
...
for (let grade of [7, 8, 9, 10, 11]) {
  const t = testsData[String(grade)];
  const sumRu = t.russian.reduce((sum, q) => sum + (q.points || 1), 0);
  const sumMa = t.math.reduce((sum, q) => sum + (q.points || 1), 0);
  const sumLo = t.logic.reduce((sum, q) => sum + (q.points || 1), 0);
  console.log("Grade", grade, "-> Ru:", sumRu, "Ma:", sumMa, "Lo:", sumLo);
}
